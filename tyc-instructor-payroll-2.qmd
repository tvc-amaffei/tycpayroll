---
title: "April 2026 Monthly Payroll"
format: 
   pdf:
    output-file: "april-2026-payroll-log"
    output-ext: "pdf"
    geometry:
      - top=30mm
      - left=20mm
...
# Clear workspace 
# Melissa's Google ID --> melissajlorusso@gmail.com
...
services_held <- read_csv(
  ...
  #  s_staff.text_fullname = col_character(),
    ...
    `Total Pay` = col_character(),
    Booked = col_integer(),
    Attended = col_integer(),
    `No Shows` = col_integer(),
    `Late Cancels` = col_integer(),
    `Pay Rate` = col_character(),
  )
)
...
services_held <- services_held |>
    mutate(wl_calc_pay = parse_number(`Total Pay`)) |>
...
combined_sv <- combined_sv |>
  ...
  mutate(total_attendees = at.s + lc.s + ns.s + at.v)
```
...
services_data_all <- combined_sv_w_pay |>
  select(instructor, date, time, short_service.s, service_type, 
         at.s, lc.s, ns.s, at.v, total_attendees, hourly_pay_rate.s, final_pay) |>
  rename(
    ...
    stype = service_type) |>
  mutate (
    short_service = str_trunc(short_service, width = 11, side = "right", ellipsis = "") 
  ) |>
  # Arrange by date and time, taking into account that date and time are character strings
  arrange( lubridate::mdy(date), 
            lubridate::parse_date_time(time, orders = "I:M p")
  )
...
for (i in 1:NROW(unique_instructors)) {
  ...
  # initialize paydata and serdata
  ...
  # render the template
    quarto::quarto_render(
        ...
        execute_params = list(
            ...
            pdata = paydatajson,
            cdata = serdatajson # Serialize the data frame
        )
    )
...
```{r move-instructor-pdfs}
#| echo: false
#| include: false

# Coerce params (YAML params often arrive as character)
yr <- as.integer(params$year)
mo <- as.integer(params$month)
ym_dirname <- sprintf("%04d-%02d", yr, mo)

# Base directories (relative to Quarto working directory)
source_dir <- file.path("quarto")
target_dir <- file.path("instructor_pdfs", ym_dirname)

# Create target directory if needed
dir.create(target_dir, showWarnings = FALSE, recursive = TRUE)

# Delete any existing PDFs in target directory
existing_pdfs <- list.files(
  path = target_dir,
  pattern = "^.*payroll-.*.pdf",
  full.names = TRUE
)
if (length(existing_pdfs) > 0) {
  file.remove(existing_pdfs)
}
...
file.rename(
from = pdf_files,
to = file.path(target_dir, basename(pdf_files))
)
}
